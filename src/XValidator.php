...
use Varobj\XP\Exception\ParamsException;
 ...
 *  - required      必须传参数, 数据必须包含 key, 如果后面类型校验存在且不是string时, 参数空字符串也可以通过,
 *                      这种情况可以使用 notEmpty 组合验证
 * 2) 校验类型
 ...
 *  - datetime      数据包含 key 时,值必须 Y-m-d H:i:s 的时间格式 eg: 2018-12-12 00:00:00 (right)
 *                      2018-12-12 00:00:0 (wrong)
 * 3) 校验内容
 *  - min:\d        数据包含 key 时,值不能大于 \d
 *  - max:\d        数据包含 key 时,值不能小于 \d
 *  - minLen:\d     数据包含 key 时,值长度不能大于 \d
 *  - maxLen:\d     数据包含 key 时,值长度不能小于 \d
 *  - enum:x,y,z    数据包含 key 时,值必须为枚举类型 (全等)
 *  - enums:x,y,z   数据包含 key 时,值必须为枚举类型 (非全等)
 *  - contains:x,y,z    数据包含 key 时,值 x,y,k 必须是 x,y,z 的子集
 *
 *  update
 *  性能从 ~35ms/千次 ->  ~22ms/千次
 */
class XValidator
{
    ...
    public function setFillNotExist(bool $fill = true): XValidator
    {
    ...
    public function validate(array $params, array $rules): array
    {
        ...
        foreach ($rules as $key => $value) {
            ...
            if (is_array($value)) {
                ...
                $subValue = $value[$keys[0]];
                $value = $keys[0];
                if (!is_array($subValue)) {
                    throw (new ParamsException('子校验必须为数组'))->debug($subValue);
                }
            }
            $default = '';
            $value = preg_replace_callback(
                '/\{(.*)\}/',
                static function ($match) use (&$default) {
                    $default = $match[1] ?? '';
                    return '';
                },
                $value
            ...
            $value = preg_replace_callback(
                '/\[(.*)\]/',
                static function ($match) use (&$desc) {
                    $desc = $match[1] ?? '';
                    return '';
                },
                $value
            ...
            $value = preg_replace_callback(
                '/<(.*)>/',
                static function ($match) use (&$error) {
                    $error = $match[1] ?? '';
                    return '';
                },
                $value
            ...
            foreach ($rule as $v) {
                ...
                if (is_callable([$this, $method])) {
                    switch (static::$_SUPPORT_MAP[$_v[0]] ?? null) {
                        ...
                        case 'type':
                            if (!empty($_v[1])) {
                                $newRule['var'][] = isset($_v[1])
                                    ? ($method . ':' . $_v[1]) : $method;
                                break;
                            }
                            if (!empty($newRule['type'])) {
                                trigger_error(
                                    '已定义过校验类型 [' . $newRule['type'] .
                                    '], 无需重复定义 type[' . $_v[0] . ']'
                                );
                            } else {
                                $newRule['type'] = $method;
                    ...
                    trigger_error('未定义的校验 method[' . $method . ']');
                }
            }
            ...
            if (!isset($this->params[$key])) {
                ...
                switch ($newRule['type'] ?? '') {
                    ...
                    case 'validateStringJsonList':
                        $this->params[$key] = $params[$key] = json_decode(
                            $this->params[$key],
                            true
                        );
                        break;
    ...
    protected function validateNotEmpty($key, $desc, bool $throw = true): bool
    {
        ...
        if ($this->params[$key] !== 0 && $this->params[$key] !== '0' &&
            empty($this->params[$key])) {
    ...
    protected function validateStringJsonList($key, $desc): bool
    {
        ...
        throw new ParamsException(
            '参数 {' . $key . ($desc ? ('}-[' . $desc . '] ') : '} ') .
            '类型必须为合法的 json list 字符串'
        );
    }

    protected function validateArray($key, $desc): bool
    {
        if (!$this->isExist($key)) {
            return true;
        }
    ...
    protected function validateArrayList($key, $desc): bool
    {
        if (!$this->isExist($key)) {
            return true;
        }
        ...
        if (is_array($this->params[$key])) {
            foreach ($this->params[$key] as $item) {
                if (!is_array($item)) {
                    throw new ParamsException(
                        '参数 {' . $key . ($desc ? ('}-[' . $desc . '] ') : '} ')
                        . '类型必须为多维数组类型'
    ...
    protected function validateMinLen($key, $desc, $value): bool
    {
        if (!$this->validateNotEmpty($key, $desc, false)) {
            return true;
        }
        if (mb_strlen(json_encode($this->params[$key])) < (int)$value) {
            throw new ParamsException(
                '参数 {' . $key . ($desc ? ('}-[' . $desc . '] ') : '} ') .
                '长度不能小于' . $value
            );
    ...
    protected function validateMaxLen($key, $desc, $value): bool
    {
        if (!$this->validateNotEmpty($key, $desc, false)) {
            return true;
        }
    ...
    protected function validateContains($key, $desc, $value): bool
    {
        if (!$this->validateNotEmpty($key, $desc, false)) {
            return true;
        }
        $values = explode(',', $value);
        $_s = explode(',', $this->params[$key]);
        if (array_intersect($_s, $values) !== $_s) {
            throw new ParamsException(
                '参数 {' . $key . ($desc ? ('}-[' . $desc . '] ') : '} ') .
                '必须为 [' . $value . '] 的子集'
            );
